...
# ========================= 新增：只加载 backbone 的辅助函数 =========================
def load_backbone_only(model, weights_path):
    """
    只从指定 .pt 权重中加载 backbone 部分 (model.0 ~ model.9)，
    其余 FPN + PAN + Detect 维持当前初始化（通常是随机初始化）。

    Args:
        model: 已构建好的 Model 实例（根据 cfg 构建）
        weights_path: 预训练权重路径，一般是 yolov5s.pt

    Returns:
        model: 加载完 backbone 权重的 model
    """
    # 确保权重文件在本地存在（支持自动下载）
    ...
    csd = ckpt["model"].float().state_dict()  # 预训练模型的 state_dict

    model_state = model.state_dict()
    new_state = {}

    # YOLOv5 backbone 一般是 model.0 ~ model.9
    ...
    LOGGER.info(f"Loaded {len(new_state)} backbone layers (model.0-9) from {weights_path}")
    if missing_keys:
        LOGGER.info(f"Backbone load - missing keys (ignored): {len(missing_keys)}")
    if unexpected_keys:
        LOGGER.info(f"Backbone load - unexpected keys (ignored): {len(unexpected_keys)}")
    return model
# ================================================================================
...
def train(hyp, opt, device, callbacks):
    ...
    pretrained = False  # 我们只在“无 cfg + 有 .pt”时当作完整预训练模型加载

    # Case 1：保持原 YOLOv5 行为 —— 没有 cfg，只给了 .pt 权重（完整恢复模型）
    if not cfg and weights.endswith(".pt"):
        pretrained = True
        with torch_distributed_zero_first(LOCAL_RANK):
    ...
    # Case 2：提供了 cfg（例如 yolov5s_p2.yaml）—— 从 cfg 构建网络；
    #         如果 weights 是 .pt，则只从该权重中加载 backbone(0~9)。
    else:
        model = Model(cfg, ch=3, nc=nc, anchors=hyp.get("anchors")).to(device)  # create from cfg
        if weights and weights.endswith(".pt"):
            LOGGER.info(f"Using backbone-only pretraining from {weights}")
            model = load_backbone_only(model, weights)
