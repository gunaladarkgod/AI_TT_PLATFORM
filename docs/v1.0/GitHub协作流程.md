...
| `4. 改进开发者` | 改进包、模块源码、参数和验证 | 基线定义、统一 Runner、平台核心代码 |
| `3. 实验操作员` | 选择已开放基线/改进包、调整允许参数、创建任务和查看结果 | 研究源码、清单定义、平台核心代码 |

平台只采用上述四个固定角色，不设置按研究方向、基线或项目细分的范围授权。建议将 `1. 平台管理员` 设为仓库管理员或维护者；`2. 基线维护者`、`4. 改进开发者` 通过功能分支和 PR 提交代码；`3. 实验操作员` 默认不授予仓库写权限，只使用平台功能。后续平台内权限实现必须与此表保持一致。
