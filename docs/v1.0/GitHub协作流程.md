...
| 角色 | 可负责的内容 | 不应直接修改的内容 |
| --- | --- | --- |
| 平台核心维护者 | 前后端公共能力、统一 Runner、引擎适配、公共配置 | 无，负责跨层集成与最终审核 |
| 基线负责人 | 所属研究方向的基线定义、源码、默认参数和实验规范 | 其他方向基线、平台核心运行代码 |
| 改进开发者 | 指定基线下的改进包、模块源码、参数和验证 | 父基线定义、统一 Runner、其他基线内容 |
| 实验人员 | 选择已开放基线/改进包、调整允许参数、创建任务和查看结果 | 研究源码、清单定义、平台核心代码 |

建议将平台核心维护者设为仓库管理员或维护者；基线负责人、改进开发者通过功能分支和 PR 提交代码；实验人员默认不授予仓库写权限，只使用平台功能。后续若在平台中实现角色权限，应与此表保持一致。
