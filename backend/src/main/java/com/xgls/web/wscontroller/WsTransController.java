...
@Slf4j
@Component
@ServerEndpoint("/working/trans/{uuid}")
@Tag(name = "转换任务WS")
public class WsTransController {
    public static ConcurrentHashMap<String, WsTransController> clients = new ConcurrentHashMap<>();
    ...
    public void onOpen(@PathParam("uuid") String uuid, Session session)
        ...
        WsTransController client = clients.remove(uuid);
        if (client != null) {
            client.close();
            client = null;
        }
