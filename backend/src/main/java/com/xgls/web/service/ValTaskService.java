...
@Service
@Slf4j
public class ValTaskService extends ServiceImpl<TrainTaskMapper, TrainTask> {
    ...
    @Async
    public void startValTask(ValParams params) {
        ...
        String valName = params.getVal_name();

        log.info("^_* start val:#{}:{}", id);
        ...
        Path valPath = tPath.resolve(CodeMap.SCRIPT_TYPE_VAL + "/" + valName);
        Path dataPath = valPath.resolve("data.yaml");
            ...
            switch (cmd) {
                case "yolo":
                    pBuilder = new ProcessBuilder(
                            ...
                            "device=" + device,
                            "batch=" + params.getBatch_size(),
                            "imgsz=" + params.getImg_size(),
                            "conf=" + params.getConf_thres(),
                            "project=" + valPath.toString(),
                            "name=" + "res",
                            "exist_ok=True");
                    /** 拓展自定义参数 */
                ...
                case "python":
                    pBuilder = new ProcessBuilder(
                            ...
                            node.toString(),
                            cmd, // python or yolo
                            script.getVal(), // val.py 地址
                            "--data", dataPath.toString(),
                            "--weights", weightPath.toString(),
                            "--device", device,
                            "--batch-size", params.getBatch_size().toString(),
        ...
        log.info("start val: {}", pBuilder.command());
        ...
        try {
            ...
            StreamGobbler errorGobbler = new StreamGobbler(cuProcess.getErrorStream(), CodeMap.SCRIPT_TYPE_VAL);
            StreamGobbler outputGobbler = new StreamGobbler(cuProcess.getInputStream(), CodeMap.SCRIPT_TYPE_VAL);
            errorGobbler.start();
            outputGobbler.start();

            // 等待进程结束并获取退出码
            int exitCode = cuProcess.waitFor();
            log.info("val process fininsh :{}", exitCode);

        } catch (IOException | InterruptedException e) {
            log.warn("val err:{}", e.getMessage());
        } finally {
            // 结束确认
            stopTrain(cuProcess);
            // 最后更新状态
    ...
    private void updateStopStatus(Integer id, Integer state) {
        ...
        record.setId(id);
        record.setVal_state(state);
        baseMapper.updateById(record);
        record.setMsg_type(CodeMap.SCRIPT_TYPE_VAL);
        // 发送状态改变消息
